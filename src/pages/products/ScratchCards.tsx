...
import Footer from "../../components/Footer";
import { Link } from "react-router-dom";
import { CreditCard, ChevronRight, ShieldCheck, Layers, Gift } from "lucide-react";

const SmartCards = () => {
  return (
    <div className="page-transition min-h-screen flex flex-col bg-gray-100 text-gray-900">
      <Navbar />

      {/* Hero Section */}
      <section className="relative pt-32 pb-16 md:pt-40 md:pb-24 bg-gradient-to-br from-indigo-600 to-purple-700 text-white text-center">
        <div className="container-custom">
          <h1 className="text-5xl font-extrabold mb-4">Scratch Cards</h1>
          <p className="text-lg max-w-2xl mx-auto opacity-90">
            Secure and innovative card solutions designed for authentication, payment, and promotions.
          </p>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="container-custom grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {[
            { title: "High Security", desc: "256-bit encryption with tamper-resistant design.", icon: <ShieldCheck className="text-indigo-600" size={28} /> },
            { title: "Versatile Applications", desc: "Compatible with payment, ID, and access control systems.", icon: <Layers className="text-indigo-600" size={28} /> },
            { title: "Scratch Card Rewards", desc: "Custom scratch cards with unique codes for promotions.", icon: <Gift className="text-indigo-600" size={28} /> }
          ].map((feature, i) => (
            <div key={i} className="bg-white p-6 rounded-lg shadow-lg flex items-center space-x-4">
              <div className="p-3 bg-indigo-100 rounded-lg">{feature.icon}</div>
              <div>
      ...
      {/* Scratch Cards Information */}
      <section className="py-16 bg-gray-50">
        <div className="container-custom">
          <h2 className="text-4xl font-bold text-center mb-10">Scratch Card Features</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {["Anti-Fraud Coating", "Unique Serial Numbers", "Custom Branding", "High-Quality Printing", "QR Code Integration", "Scratch-Resistant Material"].map(
              (feature, i) => (
                <div key={i} className="bg-white p-5 rounded-lg shadow-md flex items-center space-x-3">
      ...
      <section className="py-20 bg-gradient-to-r from-indigo-600 to-purple-500 text-white text-center">
        <div className="container-custom max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">Enhance Your Security with Our Smart & Scratch Cards</h2>
          <p className="text-lg mb-8">Contact us today to explore how our solutions can improve your business.</p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link
              to="/contact"
              className="bg-white text-indigo-600 py-3 px-8 rounded-lg font-medium hover:bg-gray-100 transition-all"
            >
              Get in Touch
            </Link>
            <Link
              to="/products"
              className="bg-white/20 text-white py-3 px-8 rounded-lg font-medium hover:bg-white/30 transition-all"
            >
              View All Products
            </Link>
          </div>
